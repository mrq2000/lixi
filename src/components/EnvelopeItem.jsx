  ...
  return (
    <button
      ...
      className={`absolute group cursor-pointer transition-all duration-300 ${
        isActive ? 'scale-150 z-[9999]' : isMobile ? 'hover:scale-[1.1]' : 'hover:scale-[1.15]'
    ...
    >
      <div className="relative">
        <img
          ...
          style={{
            filter: isActive
              ? 'drop-shadow(0 20px 40px rgba(251, 191, 36, 0.8))'
              : isMobile
        ...
        <div className={`absolute inset-0 bg-yellow-300/50 rounded-lg transition-opacity duration-300 blur-2xl -z-10 scale-110 ${
          isActive ? 'opacity-100' : isMobile ? 'opacity-0 group-active:opacity-100' : 'opacity-0 group-hover:opacity-100'
        }`}></div>
        {isActive && (
